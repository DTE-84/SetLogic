...
function MealGenerator() {
  ...
  // Auto-scroll when plan is generated
  useEffect(() => {
    ...
    try {
      ...
      setPlan("Telemetry Link Failure: Claude uplink failed. Verify API configuration.");
    } finally {
			...
			<div className='generator-header'>
				...
				<div>
					...
					<p className='generator-subtitle'>
						Comprehensive meal planning with medical contraindication checking
						and micronutrient optimization
					</p>
				</div>
			</div>
								...
								<div className='form-row'>
									...
									<div className='form-group'>
										...
										<select name='timeline' value={formData.timeline} onChange={handleChange}>
											...
											<option value='slow'>Slow & Steady (Sustainable)</option>
										</select>
									</div>
								</div>
                <div className='form-row'>
							...
							<div className='section-content'>
								<div className='form-row'>
									<div className='form-group'>
										...
										<input type='number' name='weight' value={formData.weight} onChange={handleChange} placeholder='185' required />
									</div>
									<div className='form-group'>
										<label>Height (inches)</label>
										<input type='number' name='height' value={formData.height} onChange={handleChange} placeholder='70' />
									</div>
                </div>
                <div className='form-row'>
									<div className='form-group'>
										<label>Age</label>
										<input type='number' name='age' value={formData.age} onChange={handleChange} placeholder='30' />
									</div>
                  <div className='form-group'>
                    <label>Sex</label>
              ...
              <div className='section-content'>
                <div className='form-row'>
                  <div className='form-group'>
                    ...
                    <input type='number' name='calories' value={formData.calories} onChange={handleChange} placeholder='Auto-calculate' />
                  </div>
                  <div className='form-group'>
                    <label>Protein (g)</label>
                    <input type='number' name='protein' value={formData.protein} onChange={handleChange} placeholder='Auto-calculate' />
                  </div>
                </div>
                <div className='form-row'>
                  <div className='form-group'>
                    <label>Carbs (g)</label>
                    <input type='number' name='carbs' value={formData.carbs} onChange={handleChange} placeholder='Auto-calculate' />
                  </div>
                  <div className='form-group'>
                    <label>Fat (g)</label>
                    <input type='number' name='fat' value={formData.fat} onChange={handleChange} placeholder='Auto-calculate' />
                  </div>
                </div>
              </div>
            )}
            ...
            {expandedSections.dietary && (
              <div className='section-content'>
                <div className='form-row'>
                  ...
                  <div className='form-group'>
                    ...
                    <select name='cooking_ability' value={formData.cooking_ability} onChange={handleChange}>
                      <option value='beginner'>Beginner (Microwave)</option>
                      <option value='intermediate'>Intermediate (Follow recipes)</option>
                      <option value='advanced'>Advanced (Complex cooking)</option>
                    </select>
                  </div>
                </div>
                <div className='form-group'>
                ...
                <div className='form-group'>
                  ...
                  <textarea name='food_preferences' value={formData.food_preferences} onChange={handleChange} placeholder='e.g. Love: Steak, eggs. Hate: Broccoli...' rows={2} />
                </div>
              </div>
            )}
          </div>
            ...
            {expandedSections.micros && (
              <div className='section-content'>
                <div className='form-group'>
                  ...
                  <input type='text' name='known_deficiencies' value={formData.known_deficiencies} onChange={handleChange} placeholder='e.g. Vitamin D, B12, Iron...' />
                </div>
                <div className='form-group'>
                  <label>Current Supplements</label>
                  <textarea name='current_supplements' value={formData.current_supplements} onChange={handleChange} placeholder='Include doses for redundancy checks' rows={2} />
                </div>
              </div>
            )}
          </div>
          ...
          <div className='form-section'>
            ...
            {expandedSections.lifestyle && (
              <div className='section-content'>
                ...
                <div className='form-row'>
									<div className='form-group'>
										...
										<select name='meal_prep_time' value={formData.meal_prep_time} onChange={handleChange}>
											...
											<option value='extensive'>Extensive (1+ hour/day)</option>
										</select>
									</div>
									<div className='form-group'>
				...
				{plan && (
					<div className='generator-result' ref={resultRef}>
						...
						<div className='result-content markdown-view'>
							{plan.split("\n").map((line, index) => {
								if (line.startsWith("###")) return <h4 key={index} className='text-xl font-black mt-8 mb-4 text-primary uppercase tracking-tighter'>{line.replace("###", "").trim()}</h4>;
								if (line.startsWith("##")) return <h3 key={index} className='text-2xl font-black mt-10 mb-6 text-white uppercase tracking-tighter border-b border-primary/20 pb-2'>{line.replace("##", "").trim()}</h3>;
								if (line.startsWith("#")) return <h2 key={index} className='text-3xl font-black mt-12 mb-8 text-white uppercase tracking-tight'>{line.replace("#", "").trim()}</h2>;
								if (line.startsWith("-") || line.startsWith("*")) return <li key={index} className='ml-4 mb-2 text-muted-foreground list-none flex gap-2'><span className='text-primary mt-1.5 w-1.5 h-1.5 rounded-full bg-primary shrink-0' />{line.substring(1).trim()}</li>;
